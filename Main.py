...
def set_resolution():
    return "854", "480"  # 480p resolution

 

def add_flags():
    width, height = set_resolution()
    flags = f"--encoder x264 --all-audio -s '0,1,2,3' --cfr --optimize --quality=28 --width={width} --height={height} --format=mp4 --encoder-preset=ultrafast --vb 700"
    if BURN_SUBTITLES:
